...
Three things override the default without a flag, because each one already names the tree you want: an explicit id (`job next abc12`), an explicit scope, or a `job focus` set on an issue root. `--issues` itself is forest-wide and ignores focus, the way `next all` does.
