...
Jobs is a command-line tool that keeps track of your tasks in a git-friendly event log so you can organize work from the terminal. The project is **Jobs**; the binary you invoke is `job` (singular, since each invocation acts on a job).
...
The record is `.jobs/log/*.jsonl` — one append-only JSONL file per replica, where a **replica** is one checkout on one machine (a git worktree is its own). `.jobs.db` is a disposable SQLite cache of that log: every read verb and the dashboard query it, and it can be deleted at any time. Commit `.jobs/log/`; `job gitignore` excludes the cache and its sidecars (`.jobs.db*`, which covers `-wal`, `-shm`, `.jobs.db.lock` and `.jobs.db.pre-adopt`) plus `.jobs/local.json`, this machine's replica id, clock, default identity, strict flag and focus.

Every command stats each log file against a watermark the cache records; if one grew, or a new one appeared, the cache is rebuilt by sorting the union of every file by `(ts, rep, seq)` and replaying it. Every replica gets the same tables from the same files, so there is no `job sync` — sync is `git pull`, and the next command notices. `job status` reports which happened (`cache in sync` / `cache rebuilt on open`), and `job rebuild` forces it. A rebuild that ingested another machine's events also runs a reconcile pass and prints each repair. Two replicas that minted the same short id fail the rebuild and are resolved with [`job rekey`](#database); a `.jobs.db` written before the store is converted automatically on first open, keeping `.jobs.db.pre-adopt` as a backup, deletable once you trust the result (`JOBS_NO_ADOPT=1` skips it for one invocation). The adoption note names the backup and, inside a git repository, follows with a second line: `.jobs/log` is the record now, commit it, and run `job gitignore` while a pattern is still missing. A log file declares the **store format** it was written at on its `replica` event; a file declaring a format newer than the binary knows refuses the rebuild ("the binary is older than the log"), because an event type an old binary does not know applies as a no-op and it would otherwise render the record incompletely and append to it. The one hygiene rule: never hand-edit a log file.
...
| Command | Description |
|---------|-------------|
| `job init --as <name> [--force]` | Create a `.jobs.db` in the current directory and record `<name>` as the default writer identity. `--force` overwrites an existing one. `init` always creates the database in the current directory even if an ancestor already has one — there is no silent no-op. Requires `--as` or `--strict`; there is no `$USER` fallback. `--replica-name <label>` names this checkout in the shared log — it becomes the label on the `replica` event that opens this checkout's log file, and `job status`, `job log` and `job replicas` show it beside the six-character replica id. When the directory is a git repo and `.gitignore` is missing an entry Jobs needs, prints a copy-pasteable hint pointing at `job gitignore`. See [Identity](#identity). |
| `job init --strict [--force]` | Create a `.jobs.db` with no default identity; every write must carry `--as <name>`. |
| `job gitignore [--dry-run]` | Append the missing entries (`.jobs.db*` and `.jobs/local.json`) to `.gitignore` in the database's directory, creating the file if absent. `--dry-run`/`-n` prints the same report and writes nothing. The `*` covers the WAL sidecars, the store lock and the adoption backup; `.jobs/local.json` is this machine's replica id, identity, strict flag and focus. The log files under `.jobs/log/` are the record and are meant to be committed. Idempotent; no `--as`; works before or after `init`. |
| `job merge <other.jobs.db> [--dry-run]` | Fold a diverged copy of this database into the local one. The two files must share an event prefix; unrelated databases are refused. Tasks on one side only are copied whole (labels, blocks, criteria, provenance, events); tasks on both merge per table — the task row from the later edit, labels and blocks as a union, criteria matched by short id, notes and events deduplicated. A live claim survives unless the other side closed the task. Prints a report of what won where and every claim it dropped; `--dry-run`/`-n` writes nothing, `--format=json` for the machine shape. The other file is never written. `merge` works on caches rather than through the log, so the next command adopts the result into this replica's log and keeps `.jobs.db.pre-adopt`; re-running the same merge afterwards reports `Already merged` and writes nothing, while a tail written to the other copy after the merge is refused with a pointer at the across-machines guide. Two clones of one repo need `git pull`, not `merge`. No `--as` — merge copies history rather than making it. |
| `job rebuild` | Drop every table in `.jobs.db` and replay `.jobs/log/*.jsonl` into it in `(ts, rep, seq)` order. The log is the record and the cache is disposable, so this never loses anything. Every open already rebuilds when a log file has grown, so this is the recovery verb: reach for it after a crash or when you suspect the cache. A rebuild that ingested another replica's events runs the reconcile pass — a parent whose last child closed elsewhere is closed, a child of a purged task is purged, the later of two claims made while the machines were apart is released with reason `lost-merge` — and prints each repair as an ordinary event in the log. A database that predates the store is refused, since its cache holds history no log line reproduces. No `--as`. |
| `job replicas [--format=json]` | List every replica that has written to this store: id, label, host, OS user, checkout path, event count, store format and last activity, with this checkout marked. A **replica** is one checkout on one machine; the listing is read out of the `replica` event that opens each log file, so there is no extra tracked file. No `--as` (it is a read). |
| `job replica rename <label>` | Give this checkout's replica a human-readable name by appending a fresh `replica` event. The latest event per replica wins, so the rename propagates with the log and the old name stays in the history. The default label is the machine's hostname and the checkout's path. Requires `--as`. |
...
| Command | Description |
|---------|-------------|
| `job done <id> [<id>...]` | Mark one or more tasks done, atomically. Idempotent: already-done IDs are reported, not re-recorded. If the task carries unmarked acceptance criteria, the close refuses with a `cannot close: N pending criteria` error and lists the unmarked rows — see the Acceptance criteria section below for marking and override flags. If the closed task has found-in edges to issues that are still open, the ack adds a `Surfaced:` line naming them — see below. |
...
After a successful `done`, the ack ends with a `Next:` hint naming the suggested next claimable leaf. The walk starts at the closed task's parent and, at each ancestor level, prefers forward siblings (later sort key) over earlier ones before stepping up; it only crosses into a different root tree once the closed task's own root is exhausted. Agents following the hint stay inside the current plan as long as there's work there. When it does cross, it skips issue-tree roots, matching `next` and `claim --next` — the closed task's own tree stays fully reachable whatever its kind.

If the closed task has `found-in` edges to issues that are still open (see the found-in section below), the ack prints `Surfaced: N open issues — id "title", id "title"` (singular `1 open issue`) right after that task's closing line; issues already done or canceled are not listed, and the line is omitted entirely when none are open. `done` never refuses on this account — it's provenance, not a gate. On a multi-id close, each closed task gets its own `Surfaced:` line naming only the issues it produced. `--format=json` carries the same list per closed task as `surfaced_open: [{"id": ..., "title": ...}]`.
...
| `job move <id> under <new-parent>` | Reparent. Refused for an issue-tree root: run `job kind <id> task` first, so the conversion is in the log. See [Tree kinds](#tree-kinds). |
...
Two things override the default without a flag, because each already names the tree you want: an explicit id (`job next abc12`) or an explicit scope. A [focus](#focus) is no longer a third: focus is held per kind, so a focused issue root scopes `--issues` and nothing else. `--issues` itself reads your **issue focus** first and falls back to the whole forest when none is set — the same rule the task side has always followed.

**Conversion in either direction loses nothing.** Nothing but the kind changes, and the change is in the event log (`kind task-tree → issue-tree` in `job log`). Moving an issue root *under* a parent is refused, since it would stop being a root and its kind would silently stop meaning anything — run `job kind <id> task` first.

Deliberately absent: severity, triage states, reporters. Labels cover severity and component, and `job ls --label <name>` already answers "what is broken in this component, across all plans".
...
Your **focus** is the root tree that scopes every no-argument default: bare `next`, `claim --next`, `status`'s `Next:` hint, and `orient`'s target. There is one focus per [tree kind](#tree-kinds) — a task focus and an issue focus, held independently — so triaging a bug never loses your place in the plan.

| Command | Description |
|---------|-------------|
| `job focus` | Print both, one line per kind: `Task:   <id> "<title>" — 3 of 8 done, 2 available` and `Issues: <id> "<title>" — …`, `(none)` where unset. Read-only, never requires `--as`. |
