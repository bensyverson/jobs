...
func newMergeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Fold a second copy of this database — one that was copied and then written on both machines — into the local one.\n\n" +
			"The two files must be two copies of one database. They normally share an event prefix; where they do not — because an earlier merge has since been adopted into the store — the two histories are compared as sets, and a merge whose events are all already here reports \"already merged\" and writes nothing. Two histories that overlap without lining up, and two that have nothing in common, are both refused. Tasks that exist on one side only are copied over whole, with their labels, blocks, criteria, provenance and events. Tasks both sides hold merge per table: the task row from the side with the later edit, labels and blocks as a union, criteria matched by short id with the later edit winning, and notes and events as a deduplicated union. A live claim on either side survives unless the other side closed the task, in which case the close wins and the claim is dropped.\n\n" +
