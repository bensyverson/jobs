...
- **`ui-monospace`** (data): task IDs, timestamps, event payloads, and fenced code inside a note. Resolves to SF Mono on macOS, Consolas on Windows, platform default on Linux. A monospace is required here because tabular alignment and preserved whitespace matter.
...
**Log row.** Single-column chronological event row for the Log view. Grid columns: time (mono, right-aligned) · actor (avatar + name) · verb · id-pill · title · metadata. The metadata cell (`c-log-row__meta`) is the quiet trailing payload column: a text snippet (note body, claim duration, criterion label, criteria list, label names, `task-tree → issue-tree`), or an id-pill linking to a referenced task (blocker for `blocked` / `unblocked`, source for `found in`), optionally led by a short prefix ("cleared, was"). It carries `data-event-meta="<event_type>"`, and `data-state` on `criterion_state` rows so the label picks up the Criteria section's pass/fail/skip colors. Multi-word event types collapse to a one-word verb (`found in`, `kind`, `criteria`, `criterion`, `expired`) so the verb column keeps its width. Verb is `label-caps` colored by event type — `created` uses primary, `claimed` / `unblocked` use `status-active`, `done` / `released` / `canceled` use `text-done`, `blocked` uses `status-blocked`, `noted` uses `on-surface-dim`. The row also takes a `c-log-row--<verb>` modifier that tints the *row's* title + id-pill using the same text-state tokens as Plan mode — so a glance down the feed reads as the task's state-change history without having to parse each verb. Actor column is tight (90–120px) so the verb sits close to the actor label. Most-recent rows carry a `c-log-row--new` modifier with a slide-in keyframe. The row is identical on every view that shows it. Where a cell is redundant — the actor cell on `/actors/{name}`, a page already scoped to one actor — the *list* takes a `c-log--single-actor` modifier and CSS hides the cell; hidden rather than dropped, so the row keeps its grid columns, and the row itself never varies by surface. The row has two renderers — the `log-row` template block server-side (`templates/html/partials/log_row.html.tmpl`, a partial so every view that shows events renders the identical row) and `assets/js/log-row.mjs` for events arriving over SSE — and they must emit the same markup for every event type; `TestLogRowParity_ServerAndClientMarkupMatch` and `TestLogRowParity_ActorPageMatchesClient` render both and diff them.
...
**Overflow chip (`c-filter-chip--more`).** The "+N more" chip that closes a capped filter strip. Same chip geometry, but transparent border and `on-surface-dim` text with tabular figures: it selects nothing, so it reads as the end of the row rather than one more filter. It is always a real link to the same URL with `?chips=all` — the server renders the strip expanded — and JavaScript upgrades it into an in-place expander, since the chips past the cap are already in the DOM, rendered `hidden`. The strips it closes carry the entities with an event inside the range, most-recent-first, capped at 24; the axis's leading `any` chip and the current selection are never cut. **This is the project's one overflow affordance** — a strip that outgrows its cap gets this chip, not a scroll, a dropdown, or a "show all" toggle.
...
**Timeline strip (single-actor view).** Per-lane event-density visualization on the expanded single-actor view. Five lanes (created / claimed / done / blocked / noted), each a 14px track; colored marks at `--x` percent of the selected window. The window is 24h, 7d or 30d (`?window=`, default 24h), chosen from a segmented control in the strip's header; lanes, mark positions and axis ticks all scale with it, and each window names its own tick percents so a label never sits somewhere it doesn't mean. Axis labels are positioned by the same `--x` property the marks use. Complements the scrubber's cross-actor density strip with a per-actor read.

**Tabs (`c-tabs` / `c-tab`).** `label-caps` typography. Active tab shows a 2px `primary` underline with a `full`-radius cap. Inactive tabs use `on-surface-muted` text; hover brings them to `on-surface`. **This is the project's one segmented control** — there is no second boxed/pill variant. It appears three times over: the top nav, the Plan and Issues views' Active/Archived/All show tabs, and the range selector below. Every instance is a group of plain `<a>` links inside a labelled `<nav>`, so it works with JavaScript off; the active option carries `c-tab--active` *and* `aria-current`. Reach for a new component only when the options are not navigations.
