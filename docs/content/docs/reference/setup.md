...
The two files have to be two copies of one database, and `merge` proves it by walking both event logs from the beginning: they share a prefix up to the moment one was copied, and everything after it on either side is that side's tail. When there is no shared prefix, `merge` compares the two histories as sets instead, which gives three outcomes. **Everything the other side holds is already here**: `merge` says `Already merged: all N of the other database's events are in this one, nothing to do.`, writes nothing and exits 0 — so merging the same pair twice changes nothing. **The histories overlap but no longer line up**, because this side was adopted into the store after the merge and the other copy has been written to since: `merge` refuses, because it has no shared prefix to fold that tail against; adopt the other copy in its own checkout and share the log through git instead ([across machines](../../getting-started/across-machines/)). **The histories have nothing in common**: the two files were never one database, and merging them would interleave two histories that were never one — so `merge` refuses as unrelated.
...
`merge` predates the store and works on two `.jobs.db` caches, which is what makes it the recovery path when the log is gone. It writes the merged result into the local cache rather than through the log, so the next `job` command [adopts](#adoption) it — appending the arriving events as `legacy` lines and a snapshot of the merged state to this replica's log file, and keeping `.jobs.db.pre-adopt`. That holds whether the cache was still a legacy one or had already been adopted (the `--dry-run` before the real merge adopts it): an adopted cache gets a second snapshot, placed after everything the log holds, and a rebuild from the log alone lands on the merged state. Adoption reorders the local event history by log position, which is why re-running the same merge is answered by the set comparison above rather than by a prefix; it reports `already merged` and writes nothing, and so does merging a copy that was never written to. Check the report with `--dry-run` first; if a merge went wrong, `.jobs.db.pre-adopt` is the state before it.
